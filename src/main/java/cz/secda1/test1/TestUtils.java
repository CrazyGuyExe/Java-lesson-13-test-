...
public class TestUtils {
    ...
    public static List<LunchItem> todaysMenu(List<LunchItem> items, String todayDate) {
        return LunchDBService.lunchItems.stream().filter(lunchItem -> lunchItem.getDate().equals(todayDate)).collect(Collectors.toList());
    }
    ...
    public static List<String> getAllMainDishesSorted(List<LunchItem> items) {
        return items.stream().filter(lunchItem -> lunchItem.getKind().equals(LunchItemKind.MAIN_DISH))
                .sorted(Comparator.comparing(LunchItem::getName)).map(lunchItem -> lunchItem.getName()).collect(Collectors.toList());
    }
    ...
    public static List<String> getAllMainDishesWithoutAllergens(List<LunchItem> items) {
        ...
        return items.stream().filter(lunchItem -> lunchItem.getKind().equals(LunchItemKind.MAIN_DISH)).filter(lunchItem -> lunchItem.getAllergens().size()==0)
                .map(lunchItem -> lunchItem.getName()).collect(Collectors.toList());
    }
    ...
    return items.stream().filter(lunchItem -> lunchItem.getKind().equals(LunchItemKind.DRINK))
            .filter(lunchItem -> lunchItem.getName().contains("Ovocný nápoj")|| lunchItem.getName().contains("Sirup")).map(LunchItem::getDate).collect(Collectors.toList());
    ...
    public static List<String> getAllOrderedDishes(List<LunchItem> items) {
        return items.stream().filter(lunchItem -> lunchItem.isOrdered())
                .map(lunchItem -> String.format(lunchItem.getMenuOption()+ ": " + lunchItem.getName())).collect(Collectors.toList());
    }
